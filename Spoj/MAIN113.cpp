...
ll n;
ll dp[32][4][4];
ll solve(ll i,int prev,int sprev)
{
	if(i == (n + 1)) return 1;
	if(dp[i][prev][sprev] != -1)
		return dp[i][prev][sprev];
	ll res = 0;
	if((sprev == prev) or (sprev == 0)) res += solve(i + 1,1,prev) + solve(i + 1,2,prev) + solve(i + 1,3,prev);
	else 
	{
		res += solve(i + 1,prev,prev) + solve(i + 1,sprev,prev);
	}
...
int main()
{
	...
	while(t--)
	{
		forr(i,32) forr(j,4) forr(k,4) dp[i][j][k] = -1;
		cin >> n;
