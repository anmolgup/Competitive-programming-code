...
int main()
{
	...
	// // dbg(graph);
	// for(int i=0;i<=n;i++){
	// 	for(auto child: graph[v[i]]) cout<<v[i]<<" --> "<<child<<", ";
	// 		cout<<endl;
	// }
	ll ans = 0;
	for(int i = 0;i<n;i++)
	{
		...
		{
			dfs(v[i]);
		ans += min(currsum + (k - 2)*currmin,currsum - currmin + minarray + (k - 2)*minarray + 2*(minarray + currmin));
		// dbg(ans);
		// ans += currsum + (k - 2)*currmin;
		}
	}
