...
int main()
{
	...
	vll mismatch2;
		...
		for(int i = 0;i<n;i++)
		{
			...
			// dbg(ans);
		ll totsize = mismatch2.size() + mismatch1.size();
		if(totsize % 2 != 0)
		{
			cout<<"-1";	
		}
		...
		{
			ll n1 = mismatch1.size();
			for(int i = 0;i<n1 - 1;i += 2)
			{
				ans.pb({mismatch1[i],mismatch1[i + 1]});
			}
			ll n2 = mismatch2.size();
			for(int i = 0;i<n2 - 1;i += 2)
				ans.pb({mismatch2[i],mismatch2[i + 1]});
			if(n1 % 2 == 1 and n2 % 2 == 1)
			{
				ans.pb({mismatch1[n1 - 1],mismatch1[n1 - 1]});
				ans.pb({mismatch1[n1 - 1],mismatch2[n2 - 1]});
			}
			cout<<ans.size()<<endl;
			for(auto x : ans)
				cout<<x.ff<<" "<<x.ss<<endl;
