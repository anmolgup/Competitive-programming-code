...
bool isValid(pll src)
{
	if(visited[src] == true or src.ff < 1 or src.ff > n or src.ss < 1 or src.ss > n)
		return false;
	return true;
}
pll findparent(pll src)
{
	if(parent[src] == src)
		return src;
...
ll findsz(pll src)
{
	if(parent[src] == src) 
		return sz[src];
	return findsz(parent[src]);

}
void dfs(pll a)
{
	visited[{a.ff,a.ss}] = true;
	for(int i = 0;i<4;i++)
	{
		pll curra = {a.ff + dx[i],a.ss + dy[i]};
		if(isValid(curra) and grid[curra.ff][curra.ss] == grid[a.ff][a.ss]){
			merge(a,curra);
			dfs(curra);
		}
	}
...
int main()
{
	...
	dfs(src);
	dfs(dest);
	pll psrc = findparent(src);
	pll pdest = findparent(dest);
	if(psrc == pdest){
		cout<<"0";
		return 0;
	}
	...
	for(int i = 1;i<=n;i++)
	{
		for(int j = 1;j<=n;j++)
		{
			if(findparent({i,j}) == psrc)
			{
				for(int k = 1;k<=n;k++)
				{
					for(int l = 1;l<=n;l++)
					{
						if(findparent({k,l}) == pdest)
						{
							ll currans = (i - k)*(i - k) + (j - l)*(j - l);
							ans = min(ans,currans);
						}
					}
				}
