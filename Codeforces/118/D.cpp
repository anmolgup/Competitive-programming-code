...
ll n1,n2,k1,k2;
ll dp[101][101][11][11];
ll solve(ll f,ll h,ll a,ll b){
	// if(a > k1 or b > k2) return 0;
	if(n1 == f and n2 == h) return 1;
	ll &res = dp[f][h][a][b];
	if(res != -1)
		return res;
	res = 0;
	if(f < n1 and a < k1) res += solve(f + 1,h,a + 1,0);
	if(h < n2 and b < k2) res += solve(f,h + 1,0,b + 1);
	res %= mod;
	return res;
}
int main()
