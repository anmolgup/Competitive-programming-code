	...
	while(n--)
	{
		...
		if(c == "on"){
			...
			if(a == name)
				{
					if(b == "posted")
					points[d.substr(0,d.size() - 2)] += 15;
					else
						points[d.substr(0,d.size() - 2)] += 10;
				}
			else if((d.substr(0,d.size() - 2) )== name)
			{
				if(b == "posted")
					points[a] += 15;
				else 
		...
		{
			if(a == name){
				points[c.substr(0,c.size() - 2)] += 5;
			}
			else if((c.substr(0,c.size() - 2)) == name)
				points[a] += 5;
			else
			{
				points[a] += 0;
				points[c.substr(0,c.size() - 2)] += 0;
			}
		}
	}
	// dbg(points);
