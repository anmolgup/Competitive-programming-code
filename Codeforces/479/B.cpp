...
int main()
{
	...
	for(;i<k;i++)
	{
		...
		ans.pb({highest.ss,smallest.ss});
		s.erase(it);
		s.erase(it2);
		s.insert({small + 1,smallest.ss});
		s.insert({high - 1,highest.ss});
		// dbg(smallest);
		// dbg(small);
		// ll smallest = *(s.begin())->ff;
		// ll highest = *(s.end()->ff);
		// dbg()
	}
	auto it = s.begin();
