...
int main()
{
	...
	cin>>q;
		ll a,b,c;
	vll choice(q),ps(q),qs(q);
	// vector<pair<ll,pll> >update;
	for(int i = 0;i<q;i++)
	{
		cin>>choice[i]>>ps[i];
		if(choice[i] == 1){
	...
	// dbgm(choice,ps,qs);
	ll mx = 0;
	map<ll,bool> updatedonce;
	// dbg(update);
	for(int i = q - 1;i>=0;i--)
	{	
		if(choice[i] == 1 and updatedonce[ps[i]] == false)
		{
			v[ps[i]] = max(mx,qs[i]);
			updatedonce[ps[i]] = true;
		}
		else if(choice[i] == 2)
		{
			mx = max(mx,ps[i]);
