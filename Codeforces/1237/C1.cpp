...
int main()
{
	...
	forr(i, n)
	{
		cin >> v[i].ff >> v[i].ss.ff >> v[i].ss.ss.ff;
		v[i].ss.ss.ss = i;
	}
	SORT(v);
	// dbg(v);
	...
	for (int i = 0;i<n;i++)
	{
		...
		for(int j = 0;j<n;j++)
		{
			if(done[j] == true or j == i)
				continue;
			ll x = v[i].ss.ss.ss;
			ll y = v[j].ss.ss.ss;
			ll minx = min(v[i].ff,v[j].ff);
			ll maxx = max(v[i].ff,v[j].ff);
			ll miny = min(v[i].ss.ff,v[j].ss.ff);
			ll maxy = max(v[i].ss.ff,v[j].ss.ff);
			ll minz = min(v[i].ss.ss.ff,v[j].ss.ss.ff);
			ll maxz = max(v[i].ss.ss.ff,v[j].ss.ss.ff);
			ll cnttrue = 0;
			for(int k = 0;k<n;k++)
			{
				if(k == i or k == j or done[k] == true)
					continue;
				ll currx = v[k].ff;
				ll curry = v[k].ss.ff;
				ll currz = v[k].ss.ss.ff;
				if(currx < minx or currx > maxx or curry < miny or curry > maxy or currz < minz or currz > maxz)
				{
					cnttrue++;
				}
			}
