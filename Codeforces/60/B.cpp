...
#define n_l '\n'
map<ppl,bool> visited;
ll k,n,m,x,y,cnt;
int dx[] = {0,1,0,-1,0,0};
int dy[] = {1,0,-1,0,0,0};
int dz[] = {0,0,0,0,1,-1};
char grid[11][11][11];
bool isValid(ppl src)
{
	if(src.ff < 1 or src.ff > k or src.ss.ff < 1 or src.ss.ff > n or 
		src.ss.ss < 1 or src.ss.ss > m or visited[src] == true)
		return false;
	return true;
}
void dfs(ppl node)
{
	...
	for(int i = 0;i<6;i++)
	{	
		ppl temp = {node.ff + dx[i],{node.ss.ff + dy[i],node.ss.ss + dz[i]}};
		if(isValid(temp) and grid[temp.ff][temp.ss.ff][temp.ss.ss] == grid[node.ff][node.ss.ff][node.ss.ss]){
			dfs(temp);
		}
	}
}
