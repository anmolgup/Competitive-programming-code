...
int main()
{
	...
	while(t--)
	{
		...
		vector<pair<ll,bool> > suff(n);
		string ans = "No";
		if(n < 2)
			cout<<"Yes\n";
		else
		{
			pref[0].ff = min(0ll,v[n - 1]);
			pref[0].ss = true;
			suff[n - 1].ff = min(0ll,v[n - 1]);
			suff[n - 1].ss = true;
			for(int i = 1;i<n; ++i){
				pref[i].ff = min(v[i],pref[i - 1].ff + 1);
				if(pref[i].ff > pref[i - 1].ff and pref[i - 1].ss == true)
					pref[i].ss = true;
				else
					pref[i].ss = false;
			}
			for(int i = (n - 2) ;i >=0;--i){
				suff[i].ff = min(v[i],suff[i + 1].ff + 1);
				if(suff[i].ff > suff[i + 1].ff and suff[i + 1].ss == true)
					suff[i].ss = true;
				else
					suff[i].ss = false;
			}
			...
			if(suff[0].ss == true)
				ans = "Yes";
			for(int i = 0;i<(n - 1);++i){
				if(pref[i] < suff[i + 1] and pref[i].ss == true and pref[i].ss == suff[i + 1].ss)
					ans = "Yes";
				if(pref[i] > suff[i + 1] and pref[i].ss == true and pref[i].ss == suff[i + 1].ss)
					ans = "Yes";
			}
