...
ll dfs(ll node,ll parent,ll currcost)
{
	visited[node] = true;
	ll currans = 0;
	if(g[node].size() == 1 and g[node][0].ff == parent)
		return currcost;
	for(auto child : g[node])
	{
		if(child.ff != parent and visited[child.ff] == false)
		{
			currans = max(currans,dfs(child.ff,node,currcost + child.ss));
		}
	}
	return currans;
}
