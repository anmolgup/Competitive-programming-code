...
int main()
{
	...
	for(int i = 0;i<n-1;i++)
	{
		// auto it2 = numall.begin() + i + 1;
		auto it = lower_bound(numall.begin(),numall.end(), numall[i]) - numall.begin();
		it--;
		if(it < 0 or it > n)
			continue;
		if(numall[it].ff == numall[i].ff)
		{
			cout<<"YES"<<endl;
			cout<<numall[i].ss.ff + 1<<" "<<numall[i].ss.ss + 1<<endl;
			cout<<numall[it].ss.ff + 1<<" "<<numall[it].ss.ss+1<<endl;
			return 0;
		}
	}
	cout<<"NO";
