...
int main()
{
    ...
    // dbg(pref);
    double mxavg,curravg,currsum;
    mxavg = 0;
    for(int i = 0;i<=canremove;i++)
    {
        currsum = pref[n];
        currsum -= pref[i];
        currsum += min(m - i,(n - i)*k);
        curravg = currsum/(n - i);
        // dbgm(currsum,curravg);
        mxavg = max(mxavg,curravg);
    }
    cout<<prec(10);
    cout<<mxavg;
    // try to remove elements from 1 to (n - 1) and find max. of average of them
