...
int main()
{
	...
	vector< priority_queue<ll,vll,greater<ll> > > frontshirt(3);
	for(int i = 0;i<n;i++)
	{
		cin>>a;
		a--;
		frontshirt[a].push(price[i]);
	}
	vector< priority_queue<ll,vll,greater<ll> > > backshirt(3);

	for(int i = 0;i<n;i++)
	{
		cin>>a;
		a--;
		backshirt[a].push(price[i]);
	}
	ll m;
	cin>>m;
	while(m--)
	{
		...
		ll isfound = 0;
		ll price1,price2;
		price1 = price2 = INT_MAX;
		while(frontshirt[a].size() > 0)
		{
			if(freq[frontshirt[a].top()] == 0)
				frontshirt[a].pop();
			else{
				price1 = frontshirt[a].top();
				break;
			}
		}
		while(backshirt[a].size() > 0)
		{
			if(freq[backshirt[a].top()] == 0)
				backshirt[a].pop();
			else{
				price2 = backshirt[a].top();
				break;
			}
		}
		if(price1 == price2 and price2 == INT_MAX)
		{
			cout<<"-1 ";
		}
