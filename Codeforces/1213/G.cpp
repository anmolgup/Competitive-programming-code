...
void merge(ll a, ll b)
{
	...
	if (p1 == p2)
		return;
	ll totalsz = sz[p1] + sz[p2];
	currans += ((totalsz) * (totalsz - 1)) / 2;
	currans -= (sz[p1] * (sz[p1] - 1)) / 2;
	currans -= (sz[p2] * (sz[p2] - 1)) / 2;
	if (sz[p1] > sz[p2]) {
		parent[p2] = p1;
		sz[p1] += sz[p2];
	}
...
int main()
{
	...
	for (int i = 0; i < q; i++)
	{
		while (j < (n - 1) and w[j].ff <= queries[i].ff)
		{
			merge(w[j].ss.ff, w[j].ss.ss);
			j++;
		}
		ans[queries[i].ss] = currans;
	}
	for (auto i : ans)
		cout << i << " ";
}